...
Systemet er udviklet af Søren Pilgård, og selvom det startede hos DIKUrevyen
har det allerede haft bred udbredelse blandt andet i DIKUrevyen,
SaTyRrevyen, Biorevyen, Matematikrevyen og MBKrevyen.
...
disse numre er, som regel men bestemt ikke altid, uafhængige af hinanden uden
overlap i hverken karakterer, tema eller indhold. Der sker dog ofte undtagelser
i form af f.eks. følgetoner med fortløbene handling, eller f.eks. sketches der
...
forskellige studieretninger begynder at synge ad hinanden. Dette er en god
...
time dette før eller efter en sang. Når klokken er 19.30 og backstage er klar
signalerer en boss eller instruktør til \TeX{}nikken at nu kan vi starte.
Når der ikke synges længere sætter lysmanden så revyen i gang ved at dæmpe lyset i
salen.
...
at have et pauseskilt/billede/video til at fortælle at nu er der pause.
...
Lidt før pausen er færdig begynder bandet at spille ind, de spiller typisk noget
...
numrene. Man skal altid trække den lidt med at starte, så trække den lidt mere, og så liiiige
...
slut-slutsang. Denne sidste sang handler ofte om at nu er revyen slut, men
...
har flere forestillinger men kun én efterfest kan man smide en henvisning op om at
gå på \textit{Caféen?!}

Så klapper folk af, og råber \textit{``1 - 2 - 3, \TeX{}nikken er for lækker''}
osv. Så for man langsomt genet folk ud. Til fredags forestillingen (hvis man har
to forestillinger) er der typisk et scenemøde. Til lørdags forestillingen gælder
det om hurtigst muligt at få folk ud og lukke auditoriet af, (hvis ikke I har
nogen til at hjælpe) og så skal alle skynde sig ned til Astihelvede.
...
\textit{``TeXnikken sejler''}. Det vigtigste er at holde hovedet koldt i enhver
situation. Få ro på, tænk igennem hvad der går galt, lad være med at stresse
eller opildne publikum, fokuser istedet, og få tingene tilbage på sporet.
Vi har \textbf{ALLE} været derude hvor det hele gik galt, og bagefter gik det
hele altsammen, bare husk på at holde hovedet koldt så du ikke begynder at lave
flere fejl.

Hvis du er i tvivl om et begræb så tag et kig i kapitel \ref{subsec:ordbog}.
...
taktisk røg kan han transformere stemningen og skabe en fest på scenen og i auditoriet.

\marginnote{Nogle revyer er begyndt at have en Epelipsiadvarsel,
...
til dansenumre, afspille lydeffekter såsom pistolskud og prutter. En af de
...
tiden flere funktioner til så hvis man har mod på lidt programmering er det bare at give
...
\item[Rekvisitkælderen:] Revyernes fælles opbevaringslokale, backstage under bandscenen.
  (Pas på skimmelsvamp)
\item[Dyrestalden:] En del af DIKU, gangen der ligger lige overfor når man kommer ud af Store UP1 fra
  scenen.
\item[Omklædningen:] Backstage bag de store metaldøre og indtil trædørene, her står skuespillernes ting
...
\item[Møbellageret:] Grimt udtryk som biologerne har fundet på da de ikke kan
  udtale HCØ-tryk.
...
\item[Universitetsparken 1:] Stedet der i mange år har huset DIKU (i hvertfald
  de studerende). Skal forlades lige om lidt (sådan har det været i mange år)...
...
\item[Worker] Et AV term for en af de maskiner der er tilsluttet en projektor,
  og som afvikler og viser ting.
\item[Master] Et AV term for den datamat hvorfra hele AV-systemet styres.
\item[AV-and] Dit trofaste side-kick, følger dig i medgang og modgang.
\item[AV-mand] Dig, personen [m/k] der står for at ud- og afvikle AV under revyen
\end{description}

\newpage
\subsection{Teknologier der bliver brugt}
Der bliver brugt en række forskellige teknologier til at drive AV-systemet. En
del af disse kommer du til at skulle bruge direkte, dem er der en introduktion
til her i bogen. Der er også en række teknologier nedenunder som du primært
behøver at kende til hvis du vil være med til at videreudvikle systemet.

\textbf{Emacs}\\
Emacs bliver brugt som kontrolcenter til det hele og kører på masteren.
Det er derfor væsentligt at have en hvis forståelse for hvordan Emacs virker.
Emacs er et ekstremt konfigurerbart program til at arbejde med tekst.
Emacs er efterhånden meget gammelt, (første udgave er fra 1972) og bærer på
mange måder præg af at være ældre en mange gængse konventioner.
For at gøre det lettere at bruge Emacs for en som aldrig har brugt programmet
før er der lavet en konfiguration der gør det hele lidt lettere at arbejde med.
Denne konfiguration kaldes \code{revy-simple-config}, du kan læse mere om
hvordan du bruger Emacs med denne konfiguration i kapitel
\ref{sec:simple-emacs}.
I kapitel \ref{sec:master} kan du læse mere om nogle af de revy specifikke
værktøjer du kommer til at bruge til at styre og skabe en revy.

\textbf{Emacs Lisp}\\
Emacs Lisp er det primære scripting sprog der bliver brugt.
Det bruges til at automatisere ting i Emacs, derudover kan der kaldes eksterne
kommandoer og kommunikere med andre maskiner.
Hele revysystemet bygger på en stor del Emacs Lisp, både til at styre tingene i
master, men også til at programmere effekter på workerne. Du kan læse mere om
Emacslisp på masteren i kapitel \ref{subsec:ubersicht} og mere om at bruge det på
workerne i kapitel \ref{sec:worker}.
Til simpel AV brug, uden hjemmelavede effekter behøver man dog ikke at tænke så
meget over at det er et programmeringssprog, og man kan nøjes med en begrænset
del, som denne guide nok skal introducere.

\textbf{Linux}\\
\marginnote{Det er aldrig blevet testet om det virker på andre platforme.}
AV-systemet er bygget til at køre i styresystemet linux. Workerne kører en meget
simpel opsætning (på Archlinux) hvor der kun lige kører et grafisk miljø og
revy-systemet. Hvis man skal arbejde med dem skal det gøres gennem en terminal.
I praksis burde de dog være sat op til bare at ``virke'' så man aldrig selv
behøver at gøre noget.

Masteren kører ligeledes linux, her er det dog en noget mere grafisk udgave. Det
primære værktøj er Emacs, derudover er der også en browser, adgang til filer
samt en terminal.

I kapitel \ref{sec:linux_guide} er der en gennemgang af linux, dog mest i forhold til
at bruge en terminal.

\textbf{\LaTeX}\\
Latex er et sprog beregnet til at skrive artikler. I AV-systemet kan det bruges
til at lave pæne overtekster. Her bruges Beamer samt en pakke,
\code{overtex.sty}, til at lave et slideshow.
Til den brug der er nødvendig for at lave overtekster er det relativt nemt at
lære, hvis du ikke allerede kan det. Hvis du synes Latex er for besværligt at
bruge kan du kigge mere på ubersong som er et andet format designet specifikt
til revybrug, her kan man dog ikke gøre brug af Latex pæne formatering men
bruger et andet format istedet. Læs mere om ubersong i kapitel
\ref{subsec:ubersong}.

I kapitel \ref{sec:latex} kan du læse en kort guide til Latex.\\

\textbf{C}\\
Programmet der kører på workerne er skrevet i programmeringssproget C, det
behøver du kun at tænke over hvis du vil være med til at udvikle AV-systemet

\textbf{Python}\\
AV-systemet har et par værktøjer skrevet i programmeringssproget Python.
...
\note{Reference til historien bag}
...
\end{verbatim}
Husk \code{/24} ellers vil du nok møde problemmer.
...
\begin{verbatim}
192.158.0.10    master/gateway
192.168.0.30    overtex
192.168.0.40    left
192.168.0.50    mid
192.168.0.60    right
\end{verbatim}
...
\note{Det kan være at man skal fjerne en tidligere known host}
...
Emacs er ikke bare en teksteditor men et helt miljø til at arbejde med tekst og
...
mappen til sin \code{load-path} og så indlæse ``revy''.
...
sketchen skal afspilles nogle lyde, billeder eller film, disse vil være skrevet
...
have en ``afslut'' instruktion der returnerer tilbage til det ubersict script
...
Derudover har vi adgang til en lang række funktioner der kan bruges som
...
kan der definere hvert slide, samt pauserne i disse. Når nummeret skal vises
kører en pdf fremviser på arbejderne som styres via ubertex i Emacs. Systemet
minder om uberscript, en markør viser hvilken del af latexen der vises som slide
...
Når man afvikler en sang, bliver latexen skjult af vejen så man kan fokusere på
indholdet. Husk at når man laver en ændring i latexen livestreames den ikke til
...
\subsubsection{Det virker ikke efter at jeg har opdateret}
Hust at køre \code{make clean all} på workerne efter du har opdateret dem.
...
\section{\LaTeX{} Lost}
\note{Dette er en guide til \LaTeX{} skrevet men så forsvundet fra koden igen}
En kort guide til dig der aldrig har rørt latex, og til dig der gerne vil vide
hvordan man bruger latex til at lave overtekster.

Hvad er \LaTeX{}? Latex er et sprog originalt lavet til at skrive videnskabelige
artikler. I modsætning til f.eks. Microsoft Word og Powerpoint er latex ikke et
``wysiwyg'' (What You See Is What You Get) program, det betyder at man er nødt
til at skrive sit dokument i et særligt sprog, dette kan så oversættes til en
pdf hvor det via særlige kommandoer bliver sat op som man gerne vil have det.
Man kan sammenligne det med f.eks. html som består af en række specielle
kommandoer flettet sammen med selve teksten, det kan være besværligt at læse for
en utrænet, men i browseren bliver det sat op til en pæn hjemmeside. Latex er
designet til automatisk at lave en pæn opsætning med en lækker typografi og en
erfaren AV-mand der har arbejdet med latex, vil hurtigt kunne spotte at en revy
bruger powerpoint da der er en række små detaljer hist og her hvor det ikke
bliver sat lige så pænt op (Stive publikummer opdager det desværre næppe).

På \texttt{http://www.latex-project.org/} findes den officielle side og på
\texttt{http://en.wikibooks.org/wiki/LaTeX} findes en udførlig guide til latex.
Det er dog næppe nødvendigt at læse disse sider til det behov der er nødvendigt
for at kunne lave overtekster.

Udover at kunne lave artikler er der en dokummentklasse kaldet \textit{beamer},
designet til at lave slideshows (alá powerpoint). En overtekst er i bund og
grund bare et sådant slideshow.

Øverst i en latex fil (har filnavnet \texttt{.tex}) findes ``preamblen'',
preamblen er et sæt overordnede instruktioner der definerer og importerer en
mængde funktionalitet og indstillinger for ens dokument, herfeter startes selve
dokumentet med \verb|\begin{document}| herefter kommer alt
indholdet, inklusiv diverse kommandoer til at specificere opsætningen af de
enkelte dele af teksten. Til sidst afsluttes dokumentet med
\verb|\end{document}|. Et eksempel ses i afsnit XXX.

En typisk start inklusiv preamble på overtekster ser således ud:
\begin{verbatim}
\documentclass[14pt]{beamer}
\usepackage[danish]{babel}
\usepackage[utf8]{inputenc}
...
Først sættes dokumentklassen (hvilken type dokument vi vil lave) til
\texttt{beamer} som er et slideshow. Herefter angives sproget samt
\textit{encodingen}. Pakken ``overtex'' hentes ind da den giver en række
kommandoer til let og overskueligt at skrive overteksterne.

Normalt sørger latex selv for linjeskift i de pdf filer der genereres, defor
ignoreres linjeskift i \textit{.tex}-filerne (med mindre de explicit kaldes).
Dette er en fordel i længere tekster hvor man så kan have én opsætning i den fil
man arbejder i, og en anden for brødteksten i det færdige. I en fil med mange
korte linjerkan det dog være en ulempe, vi benytter derfor \verb|\obeylines| til
at instruere latex i at alle linjeskift også skal være linjeskift i pdf'en.

\subsection{Slides}
I kroppen af tex filen kan man definere slides, dette gøres via overtex pakkens
\textit{overtex environment}.
Ved at skrive:
\begin{verbatim}
\begin{overtex}
...
\end{overtex}
\end{verbatim}
Skabes et nyt slide med indholdet mellem \texttt{begin} og \texttt{end}
kaldende.

Et slide består af alt indholdet mellem disse to dele (i dette tilfælde tre
punktummer) næste sætaf \texttt{begin} og \texttt{end} definerer det næste
slide. På denne måde kan man definere en sekvens af slides. Teksten af hvert
flaide bliver automatisk centreret.

Somme tider kan det være rart kun at vise lidt af sine slides ad gangen, til
dette kan man bruge \verb|\pause|. En pause opdeler et slide så næste del
(indtil slidet slutter eller indtil næste pause) først bliver vist når man
manuelt går videre. I den genererede pdf bliver der dermed først lavet en side
med indholdet op til pausen, og næste side indeholder så dette indhold plus det
der kommer efter pausen. Pauser er en smart måde at have mere tekst på overtexen
uden at afsløre for meget af handlingen/pointerne ad gangen.

Når man skriver et \verb|\pause| er der nødt til at være et mellemrum (eller et
linjeskift) efterfølgende (for at kunne skelne forskellige kald). Dette
mellemrum bliver dog spist af kaldet og ikke vist, hvis \verb|\pause| står til
sidst i en linje er dette ikke et problem, da der ikke vil mangle et mellemrum,
men hvis man har en pause i midten af en sætning er det ikke kønt. Derfor kan
man istedet skrive \verb|\pause{}| (bemærk de krøllede paranteser). Nu behøver
man ikke et mellemrum, og pausen ændrer ikke i opsætningen på nogen måder. Dette
kan f.eks. bruges til at indføre en pause mellem to ord, eller midt i et ord
hvis der bliver trukket meget på en enkelt stavelse. Dte er også muligt at bruge
de krøllede paranteser hver gang, årsagen til at man kan undlade dem er kun for
at spare tastetryk.

\subsection{Opsætning}
Man kan bruge en lang række forskellige kald (makroer) til at opsætte sin latex.
Her er en liste af de hyppigst anvendte.
\begin{table}[h!]
  \centering
  \begin{tabular}[h!]{l|l}
    \textit{Latex kode:} & \textit{Resultat:}\\
    \hline
    \hline
    \verb|Normal tekst| & Normal tekst\\
    \verb|\textbf{Fed tekst}| & \textbf{Fed tekst}\\
    \verb|\textit{Skrå tekst}| & \textit{Skrå tekst}\\
    \verb|\texttt{Monospace tekst}| & \texttt{Monospace tekst}\\
    \hline
    \verb|\LaTeX{}| & \LaTeX{}\\
    \verb|\TeX{}| & \TeX{}\\
    \verb|\&| & \&\\
    \verb|\textbackslash{}| & \textbackslash{}\\
    \verb|\texttildelow{}| & \texttildelow{}\\
    \verb|\-| & \-\\
    \verb|\ldots{}| & \ldots{}\\
  \end{tabular}
\end{table}
...
Latex er designet til at kunne skrive matematiske udtryk øå en pæn måde.
Overordnet er der to måder at de matematiske udtryk kan skrives på, enten kan
man lave et udtryk ``inline'' som en del af en linje
\(f(x)=\sum_{n=0}^{10}4x^2+\sqrt{\frac{x}{n}}\)
på denne måde bliver ligningen i paragraffen samme sted den er defineret,
ulempen er at nogle matematiske notationer og symboler bliver sammenpresset.
Alternativt kan man lave en selvstændig linje.
\[f(x)=\sum_{n=0}^{10}4x^2+\sqrt{\frac{x}{n}}\]
Der er to måder hver af disse kan sættes op på, en gammel metode der stammer
tilbage fra \TeX{}, men som mange stadig bruger af vane, samt en nyere defineret
i \LaTeX{}.
\begin{table}[h!]
  \centering
  \begin{tabular}[h!]{l|ll}
    \textit{Type} & \textit{Inline} & \textit{Selvstændig}\\
    \hline
    \TeX{} & \verb|$ 3 + 4 $| & \verb|$$ 3 + 4 $$|\\
    \TeX{} & \verb|\( 3 + 4 \)| & \verb|\[ 3 + 4 \]|
  \end{tabular}
  \caption{Måder at skrive matematiske udtryk}
\end{table}

Vær opmærksom på at matematiske udtryk selv forsøger at håndtere mellemrum
medmindre disse explicit udtrykkes.

I matematiske udtryk Man kan lave subscript (sænket tekst)
med \verb|x_{10}| som giver \(x_{10}\). Og superscript (hævet tekst) med
\verb|x^{10}| som giver \(x^{10}\). Hvis man udelader de krøllede paranteser er
det kun det føte tegn der hæves eller sænkes.

På den førnævnte wikiside findes en matematikside med dokumentationen af mange
gængse matemaatiske udtryk.

\subsection{Revy specifikke features}
Udover de almindelige latex kommandoer/makroer defineres et par nye i
\texttt{overtex.sty}.

\verb|\comment{}| er en kommando der får latex til at ignorere alt mellem de
krøllede paranteser.

\verb|\shell{}| er en kommando latex ignorerer, men som ubertex ser og
eksekverer som shell kode.

\verb|\elisp{}| er en kommando latex ignorerer, men som ubertex ser og
eksekverer som elisp kode.

Vær opmærksom på at ubertex scanner overteksterne igennem øw ganske primitiv vis
og vil stadig se kommandoer selvom de står i udkommmenteret kode. Generelt har
ubertex svært ved at håndtere udkommenteret latex kode, så prøv så vidt muligt
at undgå det.

Læs mere om ubertexs funktionaliteter i afsnit XXX.


\newpage
\section{Sådan bruges Emacs}
\label{sec:simple-emacs}
Dette er en guide til hvordan Emacs bruges med \code{revy-simple-config} konfigurationen.
...
har styr på linux, se evt. Appendix \ref{sec:linux_guide}.
...
Find ud af om vi er startet i UEFI mode, hvis følgende findes er passer det:
...
Dette er en gammel måde at gøre det på.
...
\begin{mdframed}[style=boxy]
  Man kan slette alt indhold på harddisken og alle eksisterende partitioner disk med
\begin{verbatim}
...
\item \texttt{1M} enter \textit{ - tidligere 1007KiB} %En gammel note foreslog 2M
...
\item \texttt{New}
\item Tryk enter, default er fint, formententligt 2048. % 6144
\item \texttt{4G} mindre swap kan også vælges (eller udelades).
...
(Ret i hvordan man bruger deet i resten af dokumentationen)

Her har vi taget udgangspunkt i at netværksinterfacet hedder \code{eth0}.

\begin{verbatim}
curl -s https://raw.githubusercontent.com/Pilen/ubertex/master/linux/revynet | tee /etc/netctl/revynet
...
Og enpXsY erstattes med interfacet.
...
% Vi ønsker ikke at man kan logge ind som root
% \begin{verbatim}
% passwd
% \end{verbatim}
% Skriv løsn til root (f.eks. \texttt{hamsterroot}).
Vi laver en \texttt{admin} bruger med sudo adgang, samt en \texttt{worker}
bruger uden. Man kan også lave en \texttt{master} bruger hvis det er nødvendigt
(Se afsnit XXX om installation af master).
...
Sudo er allerede installeret via base-devel, du kan læse mere om sudo i Appendix \ref{sec:linux_guide}.
\begin{verbatim}
EDITOR=nano visudo
\end{verbatim}
...
admin ALL=(ALL) ALL
\end{verbatim}
Brugeren \texttt{admin} kan nu sudo'e.
...
passwd worker
\end{verbatim}
Giv worker et løsn, f.eks. \texttt{hamster}

\begin{verbatim}
pacman -S openssh
\end{verbatim}
\marginnote{Det er nu default at \texttt{PermitRootLogin} er sat til \texttt{prohibit-password}, så
  det er ikke længere nødvendigt at ændre det i \texttt{/etc/ssh/sshd\_config}}
\begin{verbatim}
systemctl enable sshd.service
\end{verbatim}
% systemctl start sshd
Nu kan vi senere ssh'e ind fra vores lokale maskine \code{ssh revy@192.168.0.XXX}.
...
Du kan også logge ind som almindelig bruger istedet.

Log ind som \texttt{admin}
Flere af kommandoerne kræver nu sudo.
\marginnote{Forklar sudo, og hvad den siger ved første gang man bruger det}
Som før, hvis man ønsker et andet tastatur layout det gøres med f.eks.
\code{sudo loadkeys colemak} hvis man ønsker at slå bip-lyden fra kan man gøre det
med \code{sudo rmmod pcspkr}

\note{Hvis du som mig til jubilæumsrevyen installerede systemet
på en harddisk i en anden datamat end dens egen er det her den skal flyttes. Det
kan være at ramdisken fejler, i grub vælges da fallback løsningen og du kalder
\code{mkinitcpio -p linux} for at skabe et nyt korrekt image.}
...
ping google.com -c 4
\end{verbatim}
For at se om der er forbindelse til internettet.
...
\end{verbatim}
Tryk til alle tre spørgsmål.
Den spørger blandt andet om hvilken udbyder af \texttt{libgl} der ønskes, default (1
mesa-libgl) er formententligt et fint valg (medmindre andet ønskes).
...
ikke der findes et eller drivere til dette grafikkort (bemærk at det fører til software rendering).
\marginnote{xorg leder først efter device specifikke drivere, så prøver den med
fbdev og så vesa}
\note{mesa og mesa-libgl er installeret af xorg-server}
Til intel er driveren \texttt{xf86-video-intel}. Til Nvidia er der de opensource
\texttt{xf86-video-nouveau} eller de proprietære \texttt{nvidia} drivere. Til ATI er der
de ældre \texttt{xf86-video-ati} til gamle grafikkort og
\texttt{xf86-video-amdgpu} til de nyerere.
\note{De nye maskiner har et \texttt{ATI 102 a924b} som ifølge \texttt{lspci} er
et

% left:
\code{Display controller: Advanced Micro Devices, Inc. [AMD/ATI] RV516 [Radeon X1300/X1550 Series]}
% overtex:
\code{VGA compatiblecontroller: Advanced Micro Devices, Inc. [AMD/ATI] RV610 [Radeon HD 2400 PRO/XT]}
}

\begin{verbatim}sudo pacman -S xf86-video-fbdev xf86-video-vesa XXXX
\end{verbatim}
Hvor XXX er video driverne til det specifikke grafik kort.

\note{Det virker vidst ikke på så gamle grafikkort}
Derudover er det en god idé med Hardware video acceleration
Se \url{https://wiki.archlinux.org/index.php/Hardware_video_acceleration}
Til Intel kort brug VA-API - \texttt{libva-intel-driver},
til Nvidia brug VDPAU - \texttt{libva-vdpau-driver}?,
ATI kan bruge begge \texttt{libva-mesa-driver}
\begin{verbatim}
sudo pacman -S libva-mesa-driver libva-intel-driver libva-vdpau-driver
\end{verbatim}
...
Som windowmanager bruger vi bspwm, en simpel ``tiling'' windowmanager med
minimale vinduesdekorationer, kan styres med tastaturet og fordi jeg kender den.
Se Appendix \ref{sec:linux_guide} for mere om brugen. Til at styre (de få)
genvejstaster bruges sxhkd.
...
Bemærk den lidt anderledes konstruktion med tee, da man ikke kan sudo'e
redirects.
Outputet fra curl kan skjule forespørgslen efter løsen, så hvis den hænger er det nok derfor
\begin{verbatim}
sudo mkdir -p /etc/systemd/system/getty@tty1.service.d/
curl -s https://raw.githubusercontent.com/Pilen/ubertex/master/linux/autologin.conf
...
\begin{verbatim}
sudo pacman -S git alsa-utils rxvt-unicode dmenu xdotool feh mplayer ttf-dejavu
screen htop rsync python3
\end{verbatim}
\begin{verbatim}
sudo pacman -S cairo poppler-glib pango sdl2 sdl2_image sdl2_mixer sdl2_net
\end{verbatim}

\subsubsection{Bruger specifik konfiguration}
Følgende skal gøres lokalt som den bruger der rent faktisk viser indholdet. Så vi
skifter over til \texttt{worker}
\begin{verbatim}
sudo -iu worker
\end{verbatim}
...
\begin{verbatim}
curl -s https://raw.githubusercontent.com/Pilen/ubertex/master/linux/.xinitrc | tee .xinitrc
\end{verbatim}
...
\begin{verbatim}
curl -s https://raw.githubusercontent.com/Pilen/ubertex/master/linux/.emptycursor.xbm | tee .emptycursor.xbm
\end{verbatim}
\begin{verbatim}
curl -s https://raw.githubusercontent.com/Pilen/ubertex/master/linux/.bashrc | tee .bashrc
\end{verbatim}
\marginnote{\code{alias d='export DISPLAY=:0'} Er et praktisk lille alias.}
\begin{mdframed}[style=code]
  \verbatiminput{linux/.bashrc}
\end{mdframed}

\begin{verbatim}
mkdir -p .config/bspwm
mkdir -p .config/sxhkd
curl -s https://raw.githubusercontent.com/Pilen/ubertex/master/linux/bspwmrc | tee .config/bspwm/bspwmrc
\end{verbatim}
\begin{mdframed}[style=code]
  \verbatiminput{linux/bspwmrc}
\end{mdframed}
\begin{verbatim}
curl -s https://raw.githubusercontent.com/Pilen/ubertex/master/linux/sxhkdrc | tee .config/sxhkd/sxhkdrc
\end{verbatim}
\begin{mdframed}[style=code]
  \verbatiminput{linux/sxhkdrc}
\end{mdframed}


\begin{verbatim}
git clone https://github.com/Pilen/ubertex.git
cd ubertex/worker
make
\end{verbatim}

Du har nu installeret
\begin{verbatim}
exit
sudo reboot
\end{verbatim}
Nu genstarter workeren og burde automatisk logge ind og være klar.
...
ssh-keygen -b 4096
\end{verbatim}
tryk enter 3 gange, stedet er fint og vi vil bare have et tomt password.i
Kopier den til workerne, erstat \texttt{\$WORKERX} med den ønskede worker.
\begin{verbatim}
ssh-copy-id $WORKER1
ssh-copy-id $WORKER2
ssh-copy-id $WORKER3
...
\newpage
\section{Historien bag}
% Lidt baggrundshistorie for de nørdede.

Dette er en gennemgang af historien bag og nogle af de annekdoter der er
om AV systemet
...
grafiske grænseflader. I det sprog lavede han et \textit{dmenu} lignende program
...
server koblet til en projektor over en \texttt{ssh}-forbindelse. \textit{Xpdf} kan
...
\note{Fortæl om de gamle stationære og brok


De gamle stationærere kan overhovedet ikke følge med til krævende opgaver,
